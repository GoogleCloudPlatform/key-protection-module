...
func TestKeyHandleUUIDValidation(t *testing.T) {
	server := &grpcServer{
		svc: &mockKPS{},
	}
...
func TestKeyHandleUUIDValidation_Valid(t *testing.T) {
	req := &kpspb.DecapAndSealRequest{
		KeyHandle: &keymanager.KeyHandle{
	...
	called := false
	_, err := ValidationInterceptor(context.Background(), req, &grpc.UnaryServerInfo{}, func(ctx context.Context, r interface{}) (interface{}, error) {
		called = true
		return nil, nil
	})
